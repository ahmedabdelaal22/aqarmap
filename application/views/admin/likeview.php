...
$get_like = $this->admin_model->get_like($id);
...
<div class="content-wrapper">
	...
	<section class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1><i class="nav-icon fa fa-heart"></i> Like</h1>
				</div>
				<div class="col-sm-6">
				...
				<div class="col-12">
					...
					<div class="card card-primary">
						<div class="card-header">
							<h3 class="card-title">Likes View</h3>
						</div>
						<!-- /.card-header -->
						<div class="card-body">
							<table id="list_table" class="table table-bordered table-striped">
								...
								<tbody>
									...
									<?php if (isset($get_like)) {
										$cnt = 1; ?>
										<?php foreach ($get_like as $listing) { ?>
											<tr>
												<td><?php echo $cnt++; ?></td>
												<td><?php
													$lid = $listing->user_id;

													$query = $this->db->select('*')->from('user')->where('id', $lid)->get();
													$user = $query->row();
													if (!empty($user)) {
														echo $user->username;
													}
													?></td>
												<td><?php echo gmdate('d M Y', $listing->date); ?></td>
											</tr>
										<?php } ?>
									<?php } ?>
								</tbody>
